...
import { useUserStore } from '@/stores/UserModule/user';
...
const loginAction = async () => {
    try {
        ...
        toast.add({ severity: 'success', summary: 'Sign-in successfully', detail: 'Redirecting...', life: 3000 });
        userStore.setUser(user.data);

        setTimeout(() => {
            // Redirect to dashboard by vue router
...
<template>
    ...
    <div class="bg-surface-200 dark:bg-surface-950 flex items-center justify-center min-h-screen min-w-[100vw] overflow-hidden">
        <div class="flex flex-col items-center justify-center">
            <div style="border-radius: 56px; padding: 0.3rem; background-color: var(--primary-color); box-shadow: 0 0 20px 0 rgba(0, 0, 0, 0.15);">
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-10 px-8 sm:px-10" style="border-radius: 53px">
                    <div class="text-center mb-8">
                        ...
                        <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">Project</div>
                        <span class="text-muted-color font-medium">Sign in to continue</span>
                    </div>

                    <div class="flex flex-col gap-4 mb-4" v-if="generalError.length > 0">
                    ...
                    <div>
                        <div class="mb-5">
                            ...
                            <InputText id="email1" type="text" placeholder="Email address" class="" fluid v-model="email" autofocus :invalid="emailError.length > 0" />
                            <small v-if="emailError.length > 0" class="text-red-500">{{ emailError }}</small>
                        </div>
                        <div class="mb-5">
                            <label for="password1" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">Password</label>
                            <Password id="password1" v-model="password" placeholder="Password" :toggleMask="true" class="" fluid :feedback="false" :invalid="passwordError.length > 0"></Password>
                            <small v-if="passwordError.length > 0" class="text-red-500">{{ passwordError }}</small>
                        </div>
